...
class Project extends SettingsContainer
{
    ...
    constructor(name, parent, policy, config)
    {
        ...
        this.MergeBuildVars(config); // configure BuildVars with all config values
    ...
    IsExternal()
    {
        let rel = jsmk.path.relative(this.ProjectDir, this.m_domain.controller.ProjectDir);
        let ext = !rel.endsWith("..") && !rel.endsWith(".");
        if(ext)
            jsmk.DEBUG(`${this.ProjectDir} is external ${rel}`);
        return ext;
    }
    ...
    {
        let oldDomain; 
        if(!config || !config.overrideDomain)
            oldDomain = this.m_domain;
        this.m_domain = new ProjDomain(name, this, oldDomain,
                                       toolsetConstraints);
        if(oldDomain)
            oldDomain.AddSubDomain(this.m_domain);
        ...
        if(config)
        {
            ...
            if(config.TimestampFile)
            {
                ...
                let tsdir = jsmk.path.dirname(tsfile, {recursive: true}); // shouldn't throw
                jsmk.file.mkdirSync(tsdir, {recursive: true});
                let t = new Date();
                let ts = {
                    app: config.AppName,
                    deployment: this.GetBuildVar("Deployment"),
                    arch: this.GetBuildVar("Arch"),
                    platform: this.GetBuildVar("Platform"),
                    vers: config.Version,
                    track: config.Track,
                ...
                {
                    if(err)
                        jsmk.WARN(`${name} problem writing timestamp to ${config.TimestampFile}: ${err}`);
                    else
                        jsmk.INFO(`${name} wrote timestamp to ${config.TimestampFile}`);
                });
            }
        }
    ...
    // inline projects are those defined in a a single .jsmk file
    // rather than interspersed across the filesystem.  This allows
    // for non-intrusive build definitions (for eg open-source packages).
    // Multi-flavor builds can also be accomplished this way by adding the
    // same subproject (a directory) under different names+settings.
    //  the implementation of BuildInlineProjects is in projInline.js
    //  and it may make multiple calls to NewProject, etc.
    ...
    Glob(pat, rel = true)
    {
        ...
        let ret = jsmk.path.glob(pat);
        let rootdir = this.GetRootDir();
        if(rel)
        {
            ret.map((p) =>
            {
                return jsmk.path.relative(rootdir, p);
            });
        }
        process.chdir(old);
        return ret;
    ...
    *GenerateWork(stage) // returns a promise on each iteration
    {
        if(stage == "nuke" && this.IsDomainController())
        {
            let w = new Promise((resolve, reject) => 
            {
                jsmk.NOTICE("nuking project " + this.Name);
                // built-dir template includes module which isn't defined
                // at the project level.
                let built = jsmk.path.dirname(this.EvaluateBuildVar("BuiltDirTmplt"));
                let installed = this.EvaluateBuildVar("InstallDirTmplt");
                jsmk.NOTICE(`${this.Name} NUKING ${built}`);
                try
                {
                    jsmk.file.rmtree(built);
                }
                catch(err)
                {
                    jsmk.NOTICE(`${this.Name} NUKE error ${err}`);
                }

                jsmk.NOTICE(`${this.Name} NUKING ${installed}`);
                try
                {
                    jsmk.file.rmtree(installed);
                }
                catch(err)
                {
                    jsmk.NOTICE(`${this.Name} NUKE error ${err}`);
